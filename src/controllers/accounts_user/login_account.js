...
const loginUserAccount = async (req, res) => {
    ...
    const query = 'SELECT user_id, name, user_name, sur_name, email, password, failed_attempts, verify_email FROM user_account WHERE email = $1';
        ...
        return res.status(429).send('Has excedido el límite de intentos de inicio de sesión. Por favor, espera un momento e intenta de nuevo.');
            ...
            const passwordMatch = await compare(password, storePassword);
              ...
              if (passwordMatch) {
                ...
                try {
                  const query = 'UPDATE user_account SET token = $1, failed_attempts = 0 WHERE user_id = $2';
                ...
                const query = 'UPDATE user_account SET failed_attempts = COALESCE(failed_attempts, 0) + 1 WHERE user_id = $1;';
...
const loginUserAccountTokenGoogle = async (req, res) => {
...
const getEmailAccount = async (req, res) => {
  ...
  const query =  'SELECT user_id, name, sur_name FROM user_account WHERE email = $1';
...
const generatePasswordCodeAccount = async (req, res) => {
    ...
    try {
      ...
      let code = '';
        ...
        const now = new Date();
        const query = 'UPDATE user_account set code = $1, create_at = $2  WHERE user_id = $3;';
...
const verifyResetCodeAccount = async (req, res) => {
  const { userId, codeInto } = req.body;

  try {
    
    // Consulta la base de datos para obtener el código y la hora de creación
     const query = 'SELECT code, create_at FROM user_account WHERE user_id = $1;';
...
const updatePasswordAccount = async (req, res) => {
  ...
  try {
    ...
    const query = 'UPDATE user_account SET password = $1 WHERE user_id = $2';
...
const createdUserAccount = async (req, res) => {
  const {user_name, name, sur_name, email, password } = req.body;

  // Verificar si el usuario ya está registrado
  const userExistsQuery = await pool.query('SELECT 1 FROM user_account WHERE email = $1 LIMIT 1', [email]);
  ...
  if (userExistsQuery.rowCount > 0) {
      ...
      const query = 'INSERT INTO user_account ( email, password, user_name, name, sur_name, verify_email) VALUES ($1, $2, $3, $4, $5, false) RETURNING user_id';
      ...
      const updateQuery = 'UPDATE user_account SET token = $1, secret_key = $2 WHERE user_id = $3';
...
const verifyEmailAccount = async (req, res) => {
  try {
    ...
    // Buscar el usuario en la base de datos por correo electrónico
    const query = 'SELECT token, secret_key FROM user_account WHERE email = $1';
    const userResponse = await pool.query(query, [email]);    
    ...
    try {
      ...
      await pool.query('UPDATE user_account SET verify_email = true WHERE email = $1', [email]);
      message = 'éxito';
    } catch (err) {
      message = 'error'; 
    }
...
const createdUserAccountTokenGoogle = async (req, res) => {
...
const getAllAccountsUserAccounts = async (req, res) => {
    try {
        const query = 'SELECT user_name, email FROM user_account ORDER BY user_name ASC';
...
schedule.scheduleJob('0 0 * * *', async function(){
  try {
    await pool.query('UPDATE user_account SET failed_attempts = 0');
...
module.exports = {
    ...
    loginUserAccount,
    loginUserAccountTokenGoogle,
    createdUserAccount,
    createdUserAccountTokenGoogle,
    getAllAccountsUserAccounts,
    getEmailAccount,
    generatePasswordCodeAccount,
    verifyResetCodeAccount,
